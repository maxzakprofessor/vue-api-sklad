...
export class ParseContext {
    constructor(fileName, output, defs) {
        this._n = 1;
        this._lines = [];
    ...
    _autoReplace() {
        if (!this._lines.length || this._noReplace)
            return;
        let line = this._lines[0].replace(/<!--(.*?)-->/g, "").replace(/^([-*> \t]+)\\\\insert\s*\(([^\s\)]+)\)/gm, (_m, pre, name) => {
...
async function parse(ctx) {
    ...
    async function parseCustomProps(indent) {
        let text = "";
    ...
    async function parseHeading(indent) {
    ...
    async function parseParagraph(indent) {
    ...
    async function parseTable(indent, firstLine, props) {
    ...
    async function parseList(indent, re) {
    ...
    async function listToTable(items, props) {
        ...
        let body = items.map((r, i) => {
            if (!r.ul) {
                r = r.stack && r.stack.filter((n) => n.ul)[0];
                if (!r || !r.ul)
                    return;
            }
            r.ul.forEach((c) => {
    ...
    async function listToColumns(columns, props) {
        let widths = props.widths;
    ...
    async function parsePre(indent) {
    ...
    /** Parse a (quote) block */
    async function parseBlock(indent) {
    ...
    async function parseIncludeTag(indent, pattern, remainder) {
        ctx.shift();
        if (!pattern) {
    ...
    async function parseAny(indent) {
        let content = [];
        let start = ctx.getLineNumber();
        while (ctx.hasInput()) {
            ...
            await ctx.validate(async () => {
                if (!text)
                    ...
                    await parseCustomProps(i);
                else if (text[0] === "#")
                    content.push(await parseHeading(i));
                else if (/^\-{3,}\s*$/.test(text))
                    content.push(parseSeparator());
                else if (/^\-\s/.test(text))
                    content.push(await parseList(i, /^\s*\-\s/));
                else if (/^\*\s/.test(text))
                    content.push(await parseList(i, /^\s*\*\s/));
                else if (/^\d+\.\s/.test(text))
                    content.push(await parseList(i, /^\s*\d+\.\s/));
                else if (/^\>\s/.test(text))
                    content.push(await parseBlock(i));
                else if (/^\`\`\`/.test(text))
                    content.push(await parsePre(i));
                else {
...
export async function parseMarkdownFile(fileName, output, defs) {
