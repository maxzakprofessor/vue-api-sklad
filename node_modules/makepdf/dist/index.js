...
import { findFontFiles, getConfig, pageBreakBefore, getTableLayouts, } from "./config/index.js";
...
async function writePdf(content, ctx) {
    if (writeOutputLog)
...
function withConfig(configFileName, f) {
    let allConfig;
    if (!configFileName && existsSync(DEFAULT_CONFIG))
    ...
    if (configFileName) {
        ...
        if (!Array.isArray(overrides))
            overrides = [overrides];
        allConfig = overrides.map((o) => getConfig(o));
    }
    else {
        allConfig = [getConfig()];
    }
